...
export default function CandidateDescription() {
  ...
  const handleSubmit = (e) => {
    ...
    try {
        ...
        formData.append("announcement_title",title);
        formData.append("announcement_description",description);
        send(formData);
        } catch (error) {
...
<Sidebar />
  ...
  <h2 class="text-2xl font-bold mb-10">add policy and strategies for candidate</h2>
  ...
  <form onSubmit={handleSubmit}>
   <div class="w-full mb-4 border border-emerald-200 rounded-lg bg-emerald-50 dark:bg-emerald-700 dark:border-emerald-600">
       ...
       <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-emerald-800">
           ...
           <textarea id="editor" rows="8" style={{ transform: `scale(${scale})` }} class="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-emerald-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write an article..." required
           value={title}
            onChange={ (e) => setTitle(e.target.value)}
           ></textarea>
       </div>
   </div>
       <div class="px-4 py-2 mb-4 bg-white rounded-b-lg dark:bg-emerald-800">
           <label for="editor" class="sr-only">Publish post</label>
           <textarea id="editor" rows="8" class="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-emerald-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write an article..." required 
            value={description}
