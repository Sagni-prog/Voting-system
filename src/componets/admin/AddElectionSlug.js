...
export default function AddElectionSlug() {
    ...
    const handleSubmit = (event) => {
        try{
          ...
          setSuccessMessage('Election Slug added successfully.');
          setErrorMessage('');
          setTimeout(() => {
            setSuccessMessage('');
          }, 4000);
          ...
          setSuccessMessage('');
          setErrorMessage('Failed to add Election Slu.');
          setTimeout(() => {
            setErrorMessage('');
          }, 4000);
        }
...
<Sidebar />
  ...
  <h2 class="text-2xl font-bold mb-10">Add Election Slug For This Year</h2>
  <div className='mb-2'>
